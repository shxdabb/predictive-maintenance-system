...
def predict_maintenance(features):
    ...
    maint_proba = clf_model.predict_proba([features])
    ...
    'Maintenance Probability': maint_proba[0][1]  # probability of class 1 (needs maintenance)
    }
...
#with st.expander("🔍 Debug: Class Distribution"):
   # st.write(original_data['maintenance'].value_counts())
...
if selected == "Home":
    ...
    st.write(original_data.head(10))  # Show unscaled data

    with st.expander("🔧 Rows with Maintenance = 1"):
        st.write(original_data[original_data['maintenance'] == 1].head(10))
    ...
    if st.button('Generate Random Values'):
        sensor_1 = np.random.uniform(original_data['sensor_1'].min(), original_data['sensor_1'].max())
        sensor_2 = np.random.uniform(original_data['sensor_2'].min(), original_data['sensor_2'].max())
        sensor_3 = np.random.uniform(original_data['sensor_3'].min(), original_data['sensor_3'].max())
        operational_hours = np.random.uniform(original_data['operational_hours'].min(), original_data['operational_hours'].max())
    ...
    sensor_1 = st.number_input('Sensor 1', min_value=-100.0, max_value=100.0, value=0.0, step=0.1)
    sensor_2 = st.number_input('Sensor 2', min_value=-100.0, max_value=100.0, value=0.0, step=0.1)
    sensor_3 = st.number_input('Sensor 3', min_value=-100.0, max_value=100.0, value=0.0, step=0.1)
    operational_hours = st.number_input('Operational Hours', min_value=0.0, max_value=10000.0, value=0.0, step=1.0)
    ...
    if 'input_features' not in st.session_state:
        ...
        prediction = predict_maintenance(scaled_input[0])
